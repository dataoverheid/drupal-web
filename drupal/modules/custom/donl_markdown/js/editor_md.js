(function ($, Drupal, editormd) {
  ...
  var heading = function (size) {
    return function () {
      ...
      var previousHeading = [].concat(line.match(/^(#+)\s/)).filter(Boolean)[1] || '';
      var lineValue = line.replace(/^(#+)\s/, '');
  ...
  Drupal.editors.editor_md = {
    ...
    getToolbarIcons: function (toolbarIcons, settings) {
      ...
      if (typeof toolbarIcons === 'object' && Object.prototype.toString.call(toolbarIcons) === '[object Array]') {
        icons = toolbarIcons;
      }
