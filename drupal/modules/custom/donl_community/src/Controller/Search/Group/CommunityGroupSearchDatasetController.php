...
namespace Drupal\donl_community\Controller\Search\Group;
...
use Drupal\donl_search\Controller\Group\GroupSearchDatasetController;
...
/**
 * Search datasets within a group within the community.
 */
class CommunityGroupSearchDatasetController extends GroupSearchDatasetController {
  ...
  public function view(NodeInterface $group, $page, $recordsPerPage): array {
    ...
    $build = parent::view($group, $page, $recordsPerPage);

    $title = $this->t('Back to all @type in community :community', [
      '@type' => $this->t('popular groups of datasets'),
      ':community' => $community->getTitle(),
    ]);
    $routeParams = ['community' => $community->getMachineName()];
    $build['#backLink'] = $this->backLinkService->createBackLink($title, 'donl_community.search.group', $routeParams);
